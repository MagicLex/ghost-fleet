"""Deploy GHOST FLEET as a custom (FastAPI) Hopsworks app.

Thin client on stock python-app-pipeline: fastapi/uvicorn/websockets uv-installed
at pod start, no clone (nothing heavy loads in this pod). Redeploy uses the full
recovery sequence (stop, purge lingering k8s deployment, drain, stop zombie
executions, settle) -- app.stop() returns before the execution dies.
"""
import subprocess
import time
...
ENTRYPOINT = ('bash -lc "python -m uv pip install --system --no-cache '
              "'fastapi==0.139.0' 'uvicorn==0.49.0' 'websockets' && "
...
def main():
    ...
    if app is None:
        app = apps.create_app(
            ...
            description="GHOST FLEET -- live shadow-fleet exposure over the "
                        "Baltic: suspicion heat map, dark/STS hotspots, network")
    else:
        try:
            app.stop()
        except Exception:
